...
def main():
    ...
    # print(train.head(50))
    train.to_csv("../BLSTM_CRF/input/train_input_BLSTM_CRF.tsv", index=False, header=False, sep="\t")

    # Process test
    text_test = get_texts(df_test)
    test = get_input(text_test, loc, char)
    # print(test.head(50))
    test.to_csv("../BLSTM_CRF/input/test_input_BLSTM_CRF.tsv", index=False, header=False, sep="\t")

    # Process dev
    text_dev = get_texts(df_dev)
    dev = get_input(text_dev, loc, char)
    # print(dev.head(50))
    dev.to_csv("../BLSTM_CRF/input/dev_input_BLSTM_CRF.tsv", index=False, header=False, sep="\t")
